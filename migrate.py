...
def migrate():
    ...
    with get_db_conn() as conn:
        ...
        # Migrate users
        if os.path.exists(USER_SETTINGS_FILE) and not USER_SETTINGS_FILE.endswith(
            ".bak"
        ):
            try:
                with open(USER_SETTINGS_FILE, "r") as f:
                    ...
                    for user_id, s in settings.items():
                        with db_lock:
                            cursor.execute(
                                """
                                INSERT OR IGNORE INTO users (user_id, city, time, timezone, subscribed, initialized, lang, weather_mode)
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                            """,
                                (
                                    user_id,
